...
logging.basicConfig(
    filename =  config.LOG_DIR/'rtm_long.txt',
    level = logging.INFO,
...
try:
    ...
    check_date = yesterday.strftime("%d-%m-%Y")
    master_df = pd.DataFrame()
    logging.info("Opening URL...")
    driver.get("https://www.iexindia.com/market-data/real-time-market/market-snapshot")
    ...
    try:
        ...
        logging.info("DELIVERY PERIOD dropdown found. Selecting 'Yesterday'...")
        option = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//li[text()='Yesterday']"))
        )
        logging.info("Yesterday selected.")
        driver.execute_script("arguments[0].scrollIntoView(true);", option)
        option.click()
    except NoSuchElementException as e:
    ...
    try:
        ...
        try:
            ...
            logging.exception(f'Error while waiting for table load:{e}')
        ...
        file_path = 'Real_Time_Market.csv'
    ...
    master_df.to_html(config.REPORT_DIR/"RTM_summary.html",index = False)
    master_df.to_excel(config.REPORT_DIR/f"RTM_summary{check_date}.xlsx",index = False)
...
ax2.plot(df_plot["Hour"], df_plot["Weighted MCP (Rs/MWh)"], 
         color='red', marker='o', label="Weighted MCP")

ax2.set_ylabel("Price (Rs/MWh)", color='yellow')
...
plt.title("IEX RTM: Price vs Volume (Hourly)")
plt.grid(True)
plt.savefig(config.PLOT_DIR/"rtm_plot.png")
plt.close()
...
html_file_path = config.REPORT_DIR/"RTM_summary.html"
image_path = config.PLOT_DIR/"rtm_plot.png"
...
try:
    ...
    mail.To =  "abhishek.agarwal@feplglobal.com;keshav.gayakwad@feplglobal.com;gunjan.chaudhari@feplglobal.com"
    mail.Subject = f"Real Time Market Report:{check_date}"
    ...
    Please find the <b>Real Time Market Summary</b> below:<br><br>
